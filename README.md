...
Um jogo de Sudoku completo e modular desenvolvido em Java, focado em demonstrar conceitos de Orientação a Objetos, Clean Code e uso eficiente da API de Streams do Java. Utiliza a Interface de Linha de Comando (CLI), gerada no terminal do seu eeditor de código, para jogabilidade.
...
* * **IntelliJ IDEA** (Configuração de módulo incluída).
...
sudoku.iml: Arquivo de configuração de módulo do IntelliJ.

## 💻 Configuração na IDE (IntelliJ IDEA)

Este repositório inclui o arquivo `sudoku.iml`, que facilita a configuração instantânea do ambiente de desenvolvimento.

1.  **Abra o IntelliJ IDEA**.
2.  Clique em **Open** (ou **File > Open** no menu).
3.  Navegue até a pasta do projeto e selecione o arquivo `sudoku.iml` (ou o diretório raiz).
4.  Clique em **OK**.
5.  A IDE reconhecerá automaticamente a pasta `src` como a fonte do projeto e configurará o SDK Java.
...
2.  No terminal, execute a classe `Main` passando a seguinte string de argumentos:
...
### Observação: Se estiver usando a IDE IntelliJ ou Eclipse, adicione a string acima no campo "Program Arguments" nas configurações de execução.
